...
import { fetchStoreOrders, type OrderSummary } from "@/lib/agent-api";
...
/// Same connected store as /seller/new-advance and /seller — see that
/// page for why: one of the agent service's real fixture stores, kept
/// distinct from harlow-and-finch (used for the bootstrap investor-flow
/// vault) so the two demo threads stay independent.
...
function ReceivableRow({ order }: { order: OrderSummary }) {
  return (
    <tr>
      ...
      <td className="px-4 py-3">
        <DeliveryScanMark scanned={order.has_delivery_scan} />
      </td>
    </tr>
  );
}

function ReceivableCard({ order }: { order: OrderSummary }) {
  return (
...
export default function ReceivablesPage() {
  ...
  useEffect(() => {
    ...
    fetchStoreOrders(CONNECTED_STORE_ID)
      .then((response) => {
        if (!cancelled) setState({ status: "ready", orders: response.orders });
      })
      .catch((error: unknown) => {
        if (!cancelled) {
          setState({
  ...
  const scannedCount = orders.filter((order) => order.has_delivery_scan).length;
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-0">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1.5 text-sm text-muted-foreground">
            Orders from {storeDisplayName(CONNECTED_STORE_ID)}, eligible to advance against.
          </p>
        </div>
      ...
      {state.status === "loading" ? (
        <p className="mt-8 text-sm text-muted-foreground">Loading orders from the agent service…</p>
      ) : (
        <>
